...
static void countdown_window_load(Window *window) {
  ...
  s_ready_text_layer = text_layer_create(GRect(0, bounds.size.h / 2 + 35, bounds.size.w, 40));
  ...
  #ifdef PBL_PLATFORM_EMERY
  text_layer_set_font(s_ready_text_layer, fonts_get_system_font(FONT_KEY_GOTHIC_24_BOLD));
  #elif defined (PBL_PLATFORM_GABBRO)
  text_layer_set_font(s_ready_text_layer, fonts_get_system_font(FONT_KEY_GOTHIC_24_BOLD));
  #else
  ...
  if(reason != -1 && persist_read_int(PERSIST_READY) != 0) {
    ...
    switch(persist_read_int(PERSIST_READY)) {
      ...
      case 4:
        ...
        break;
      case 5:
        // Custom cooldown — added on top of the tea temperature adjustment (same base as option 1)
        delay += persist_read_int(PERSIST_READY_CUSTOM);
        break;
    }
