...
// Index value used to identify the custom tea slot
#define CUSTOM_TEA_INDEX 9
...
// Buffer for custom tea name read from persist storage
static char s_custom_name[16];
...
static TeaInfo tea_array[] = {
  {"Black",   240, PERSIST_TEA_BLACK,   PERSIST_HIDE_BLACK,   96},
  {"Green",   120, PERSIST_TEA_GREEN,   PERSIST_HIDE_GREEN,   80},
  {"Herbal",  240, PERSIST_TEA_HERBAL,  PERSIST_HIDE_HERBAL,  96},
  {"Maté",    240, PERSIST_TEA_MATE,    PERSIST_HIDE_MATE,    85},
  {"Matcha",   30, PERSIST_TEA_MATCHA,  PERSIST_HIDE_MATCHA,  75},
  {"Oolong",  240, PERSIST_TEA_OOLONG,  PERSIST_HIDE_OOLONG,  85},
  {"Pu'erh",  240, PERSIST_TEA_PUERH,   PERSIST_HIDE_PUERH,   96},
  {"Rooibos", 240, PERSIST_TEA_ROOIBOS, PERSIST_HIDE_ROOIBOS, 96},
  {"White",   240, PERSIST_TEA_WHITE,   PERSIST_HIDE_WHITE,   90}
};
...
static void menu_draw_row(GContext *ctx, const Layer *cell_layer, MenuIndex *cell_index, void *callback_context) {
  ...
  if (index == CUSTOM_TEA_INDEX) {
    // Custom tea — read name, time and temp from persist storage
    persist_read_string(PERSIST_CUSTOM_NAME, s_custom_name, sizeof(s_custom_name));
    name = s_custom_name;
    steep_time = persist_read_int(PERSIST_CUSTOM_TIME);
    temp = persist_read_int(PERSIST_CUSTOM_TEMP);
  } else {
    name = tea_array[index].name;
    int persist_key = tea_array[index].persist_key;
    steep_time = (persist_read_int(persist_key) != 0 ? persist_read_int(persist_key) : tea_array[index].def_time);
    temp = tea_array[index].temp;
  }
...
// A built-in tea is visible if:
//   - its hide key has never been set (first run, show everything), OR
//   - its hide key is explicitly 1 (user chose to show it)
static bool tea_is_visible(int i) {
  int hide_key = tea_array[i].hide_key;
  return !persist_exists(hide_key) || persist_read_int(hide_key) == 1;
}

// The custom tea is visible if a non-empty name has been stored
static bool custom_tea_is_visible() {
  if (!persist_exists(PERSIST_CUSTOM_NAME)) return false;
  persist_read_string(PERSIST_CUSTOM_NAME, s_custom_name, sizeof(s_custom_name));
  return s_custom_name[0] != '\0';
}
...
static int get_tea_index_by_pos(int position) {
  ...
  // Check custom tea slot
  if (custom_tea_is_visible()) {
    if (count == position)
      return CUSTOM_TEA_INDEX;
  }
...
int get_tea_temp(int index) {
  if (index == CUSTOM_TEA_INDEX)
    return persist_read_int(PERSIST_CUSTOM_TEMP);
